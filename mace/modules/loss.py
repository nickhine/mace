...
# ------------------------------------------------------------------------------
# Dipole and Polarizability Loss Functions
# ------------------------------------------------------------------------------
...
def weighted_mean_squared_error_dipole_deriv(
    ref: Batch, pred: TensorDict, ddp: Optional[bool] = None) -> torch.Tensor:
    # dipole_deriv: [n_graphs, n_atoms, ]
    raw_loss = torch.square((torch.reshape(ref['dipole_deriv'], pred["dipole_deriv"].shape) - pred['dipole_deriv']) )
    return reduce_loss(raw_loss, ddp)
