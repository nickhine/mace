...
def get_dataset_from_xyz(
    ...
    for i, path in enumerate(train_paths):
        ...
        logging.info(
            ...
            f"{np.sum([1 if getattr(config, 'stress', None) is not None else 0 for config in train_configs])} stresses, "
            f"{np.sum([1 if config.dipole is not None and (config.dipole != 0.0).any() else 0 for config in train_configs])} dipoles]]"
        )
    ...
    if valid_paths:
        for i, path in enumerate(valid_paths):
            ...
            logging.info(
                ...
                f"{np.sum([1 if config.dipole is not None and (config.dipole != 0.0).any() else 0 for config in train_configs])} dipoles] loaded from '{path}'"
            )
        ...
        logging.info(
            ...
            f"{np.sum([1 if getattr(config, 'forces', None) is not None else 0 for config in valid_configs])} forces, "
            f"{np.sum([1 if getattr(config, 'stress', None) is not None else 0 for config in valid_configs])} stresses, "
            f"{np.sum([1 if config.dipole is not None and (config.dipole != 0.0).any() else 0 for config in valid_configs])} dipoles]]"
        )
    ...
    if test_paths:
        ...
        for name, tmp_configs in test_configs_by_type:
            logging.info(
                ...
                f"{np.sum([1 if config.dipole is not None and (config.dipole != 0.0).any() else 0 for config in tmp_configs])} dipoles"
            )
